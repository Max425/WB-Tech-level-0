...
func (r *ItemRepository) GetById(id int) (*model.Item, error) {
	...
	query := fmt.Sprintf("SELECT * FROM %s WHERE item_id = $1", constants.ItemTable)
...
func (r *ItemRepository) Update(id int, updatedItem *model.Item) error {
	query := fmt.Sprintf(`
		UPDATE %s
		SET chrt_id=$1, track_number=$2, price=$3, rid=$4, item_name=$5, sale=$6, size=$7, total_price=$8,
		nm_id=$9, brand=$10, status=$11
		WHERE item_id = $12`,
		constants.ItemTable)
	...
	_, err := r.db.Exec(query, updatedItem.ChrtID, updatedItem.TrackNumber, updatedItem.Price, updatedItem.RID,
		...
		updatedItem.Brand, updatedItem.Status, id)
	if err != nil {
...
func (r *ItemRepository) Delete(id int) error {
	query := fmt.Sprintf("DELETE FROM %s WHERE item_id = $1", constants.ItemTable)
